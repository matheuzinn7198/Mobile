...
class _UsuarioListViewState extends State<UsuarioListView> {
  ...
  _load() async{
    ...
    try {
      _usuarios = await _controller.fetchAll(); //preenche a lista de usuário com os usuários
  ...
  void _filtrar(){ //filtrar da lista já carregada
    ...
    setState(() {
      _usuariosFiltrados = _usuarios.where((user){
        ...
        user.email.toLowerCase().contains(busca); //filtro pelo email
      }).toList(); //converte em lista
    });
  }

  //criar o método deletar
  void _delete(UsuarioModel user) async {
    if(user.id == null) return; //se o id for nulo não faz nada, interrompe o método
  ...
  void _openForm({UsuarioModel? user}) async{ //usuário entra no parâmetro, mas não é obrigatório
    ...
    return Scaffold(
      ...
      : Column(
        children: [
          ...
          Divider(),
          Expanded(
            child: ListView.builder(// mostra a lista de usuários
